...
# TODO: allow x to be multivariate -> X
    ...
    Rmax : array, shape (n_perms), optional
        The Rmax distribution sampled under null hypothesis, returned only when
        return_dist == True.
    ...
    if corr in ['pearson', 'pearsonr']:
        ...
        raise ValueError('Invalid value for corr="{}".'.format(corr))
...
def permutation_pearsonr(*args, **kwargs):
    raise ValueError('This function does not exist. Use permutation_corr with '
                     'corr="pearson".')
...
def permutation_ttest_rel(X, Y, *,
    ...
    tmax : array, shape (n_perms), optional
        The tmax distribution sampled under null hypothesis, returned only when
        return_dist == True.
...
def permutation_wilcoxon(X, Y, *,
    ...
    Tmin : array, shape (n_perms), optional
        The Tmin distribution sampled under null hypothesis, returned only when
        return_dist == True.
    ...
    if not zero_method in ['wilcox', 'pratt', 'zsplit']:
        raise ValueError('Invalid value for zero_method="{}".'
                         .format(zero_method))
...
def permutation_mannwhitneyu(X, Y, *,
    ...
    Umin : array, shape (n_perms), optional
        The Umin distribution sampled under null hypothesis, returned only when
        return_dist == True.
...
# TODO: allow n='all'
    ...
    Fmax : array, shape (n_perms), optional
        The Fmax distribution sampled under null hypothesis, returned only when
        return_dist == True.
...
# TODO: allow n='all'
    ...
    Hmax : array, shape (n_perms), optional
        The Hmax distribution sampled under null hypothesis, returned only when
        return_dist == True.
...
# generalization to S samples
    ...
    chi2max : array, shape (n_perms), optional
        The chi2max distribution sampled under null hypothesis, returned only
    ...
    n_groups = len(args)
    if len(args) < 3:
        raise ValueError('Need at least three groups, got {}.'
                         .format(n_groups))
    if args[0].ndim < 2:
